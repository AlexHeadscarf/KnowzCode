...
* **Spec-Driven Development with "As-Built" Accuracy**: `knowzcode/specs/[NodeID].md` files are central. They are drafted using the strict template herein, approved, implemented against, and then finalized to an "as-built" state post-verification.
...
This document outlines the strategic process (`what` to do). The tactical execution (`how` to do it), including specific commands for file operations, version control, testing, and timestamp generation, is determined by your **`knowzcode/environment_context.md`**. You are expected to use the appropriate tools for your environment to translate the abstract steps in this loop into concrete actions.

### 2.2. Core Files Reference & Interaction Protocols
* **`knowzcode/knowzcode_project.md`**: Read-only for context.
* **`knowzcode/knowzcode_architecture.md`**: Read for context. Modify for simple consistency updates autonomously.
* **`knowzcode/knowzcode_tracker.md`**: Modify the `Status` and `WorkGroupID` columns for Change Sets; add `REFACTOR_` tasks.
* **`knowzcode/knowzcode_log.md`**: Prepend new log entries using the format defined herein; read "Reference Quality Criteria".
* **`knowzcode/specs/[NodeID].md` files**: Create, read, and finalize specs to "as-built" status using the format defined herein.
* **This `knowzcode/knowzcode_loop.md` document**: Your primary set of instructions.
* **`knowzcode/environment_context.md` (or equivalent)**: Your guide to the *how* for your specific platform.
* **`knowzcode/automation_manifest.md`**: Directory of available slash commands, subagents, skills, and hooks that reinforce every loop phase.
* **`knowzcode/workgroups/<WorkGroupID>.md`**: Persistent todo list for the active Change Set. Every entry must begin with `KnowzCode:` so automation can surface the tasks after each command.
...
2.2. **Refresh Environment Protocol**: Review `knowzcode/environment_context.md` to load the specific commands and protocols for the current platform.

**Step 3: Specification Management**
3.1. Draft or refine the `knowzcode/specs/[NodeID].md` files for the nodes in the Change Set.
3.2. **Draft `knowzcode/specs/[NodeID].md` Content**: The content for any new or updated spec MUST use the following template. The `Timestamp` MUST be sourced from the environment.
    ```markdown
    ...
    * [**CRITICAL for Tech Debt**: Any assumptions, potential challenges, or **observed technical debt** that should be addressed later. Mirror actionable debt items in `knowzcode/workgroups/<WorkGroupID>.md` with the `KnowzCode:` prefix.]
    ```
3.3. Present the drafted/refined spec(s) to the Orchestrator for approval. **PAUSE and await Orchestrator approval.**
3.4. Log a "SpecApproved" event in `knowzcode/knowzcode_log.md` for each approved spec.

**Step 3.5: (Optional) Specification Verification Checkpoint**
*This step is triggered based on Change Set size and complexity:*
- **Automatic for Large Change Sets**: If the WorkGroupID contains ≥10 NodeIDs, the agent will recommend using `KCv2.0__Spec_Verification_Checkpoint.md`
...
- **Trigger**: Orchestrator runs `KCv2.0__[LOOP_2B]__Verify_Implementation.md` after Loop 2A claims completion
- **Process**:
...
*   For **EACH** `NodeID` in the active `WorkGroupID`, update its `knowzcode/specs/[NodeID].md` file to precisely match the verified, "as-built" implementation.

**Step 8: Flowchart Consistency Check**
8.1. Review `knowzcode/knowzcode_architecture.md` against the entire "as-built" Change Set.
...
**Step 9: Log Operation in `knowzcode/knowzcode_log.md`**
9.1. After all specs are finalized and the flowchart is checked, prepend a **single**, comprehensive `ARC-Completion` entry to `knowzcode/knowzcode_log.md`.
9.2. **Log Entry Structure**: The entry **MUST** use the following Markdown format. The `Timestamp` **MUST** be generated by executing the timestamp command from your `knowzcode/environment_context.md`.
    ```markdown
    ---
    **Type:** ARC-Completion
    ...
    **Details:**
    ...
    - **Project Overview Updates:** [None | List sections updated in knowzcode_project.md].
    ---
    ```
...
10.2. **Technical Debt Scheduling**: After marking a node `[VERIFIED]`, review its finalized spec. If significant technical debt was documented, create a new `REFACTOR_[NodeID]` task in `knowzcode_tracker.md`.
10.3. **Project Overview Check**: Review if this Change Set impacts `knowzcode_project.md`:
...
4.3. **Log** the action by prepending a `MicroFix` entry to `knowzcode/knowzcode_log.md`. The entry **MUST** use the following format and an environment-sourced timestamp:
    ```markdown
    ---
    **Type:** MicroFix
    **Timestamp:** [Generated Timestamp]
